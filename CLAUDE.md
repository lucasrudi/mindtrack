...
- Examples: `feat(auth): add JWT token versioning`, `fix(security): resolve SonarCloud findings`
- Link the issue in the PR body: `Closes #N` (on its own line) — this auto-closes the issue on merge and is used by release-please for changelogs
